...
public:
    ...
    FastAdjacencyList(const FastAdjacencyList& other) {
        adj_lists.resize(other.adj_lists.size());
        for (size_t i = 0; i < other.adj_lists.size(); ++i) {
            adj_lists[i] = other.adj_lists[i];
        }
    }
    
    FastAdjacencyList& operator=(const FastAdjacencyList& other) {
        if (this != &other) {
            adj_lists.resize(other.adj_lists.size());
            for (size_t i = 0; i < other.adj_lists.size(); ++i) {
                adj_lists[i] = other.adj_lists[i];
...
public:
    ...
    Graph(const Graph& other) : adj(other.adj), core(other.core) {}
...
public:
    ...
    void fast_promote_core_1_vertices(std::unordered_set<vertex_t>& candidates, 
        ...
        // 并行处理
        std::vector<char> to_promote(core_1_candidates.size(), 0);
...
// 核心度正确性验证器
class CoreNumberValidator {
public:
   // 验证两个图的核心度是否一致
   static bool validate_core_numbers(const Graph& g1, const Graph& g2) {
       if (g1.core.size() != g2.core.size()) {
           std::cerr << "Graph sizes do not match: " << g1.core.size() << " vs " << g2.core.size() << std::endl;
           return false;
       }
       
       for (size_t v = 0; v < g1.core.size(); ++v) {
           if (g1.core[v] != g2.core[v]) {
               std::cerr << "Core number mismatch at vertex " << v << ": " 
                         << g1.core[v] << " vs " << g2.core[v] << std::endl;
               return false;
           }
       }
   ...
   // 计算两个图核心度的差异统计
   static void report_differences(const Graph& g1, const Graph& g2, const std::string& method1, const std::string& method2) {
       if (g1.core.size() != g2.core.size()) {
           std::cerr << "Graph sizes do not match: " << g1.core.size() << " vs " << g2.core.size() << std::endl;
           return;
       }
       ...
       std::vector<int> diff_vertices;
       
       for (size_t v = 0; v < g1.core.size(); ++v) {
           if (g1.core[v] != g2.core[v]) {
               mismatch_count++;
               if (diff_vertices.size() < 10) { // 只存储前10个不匹配的顶点
                   diff_vertices.push_back(v);
               }
           }
       }
       
       if (mismatch_count > 0) {
           std::cout << "Core number mismatches between " << method1 << " and " << method2 << ":" << std::endl;
           std::cout << "Total mismatches: " << mismatch_count << " out of " << g1.core.size() << " vertices (" 
                     << (100.0 * mismatch_count / g1.core.size()) << "%)" << std::endl;
           
           std::cout << "First " << std::min(10, mismatch_count) << " mismatched vertices:" << std::endl;
           for (int v : diff_vertices) {
               std::cout << "Vertex " << v << ": " << method1 << "=" << g1.core[v] 
                         << ", " << method2 << "=" << g2.core[v] << std::endl;
           }
       } else {
           std::cout << "Core numbers match perfectly between " << method1 << " and " << method2 << "!" << std::endl;
       }
   }
   
   // 验证核心度是否正确（通过从头计算）
   static bool verify_core_correctness(Graph& graph, const std::string& method) {
       // 保存当前核心度
       std::vector<int> original_core = graph.core;
       ...
       for (size_t v = 0; v < graph.core.size(); ++v) {
           if (original_core[v] != graph.core[v]) {
               if (mismatch_count < 10) {
                   std::cerr << "Core number mismatch at vertex " << v << ": " 
                             << original_core[v] << " (" << method << ") vs " 
                             << graph.core[v] << " (recomputed)" << std::endl;
               }
               match = false;
               mismatch_count++;
           }
       ...
       if (!match) {
           std::cerr << "Total " << mismatch_count << " mismatches out of " << graph.core.size() 
                     << " vertices (" << (100.0 * mismatch_count / graph.core.size()) << "%)" << std::endl;
       } else {
           std::cout << "Core numbers computed by " << method << " match the recomputed values perfectly!" << std::endl;
       }
       
       // 恢复原始核心度
       graph.core = original_core;
...
// 实验运行器
class CorrectnessExperimentRunner {
private:
...
public:
   CorrectnessExperimentRunner(TemporalGraphProcessor& proc, int win_size, int start_bin, int steps, int threads)
   ...
   void run() {
       ...
       struct StepResult {
           ...
           bool exact_matches_mcdpcd;
           bool exact_matches_ucr;
       ...
       // 开始滑动窗口实验
       std::cout << "\nStarting sliding window correctness experiment..." << std::endl;
       ...
       for (int step = 0; step < num_steps; ++step) {
           ...
           // 创建图的副本用于测试
           Graph graph_mcdpcd = graph;
           Graph graph_ucr = graph;
           Graph graph_exact = graph;
           ...
           for (const auto& edge : remove_edges) {
               graph_exact.remove_edge(edge.first, edge.second);
           }
           for (const auto& edge : add_edges) {
               graph_exact.add_edge(edge.first, edge.second);
           }
           
           // 重新计算核心度
           graph_exact.compute_core_numbers();
           std::cout << "Exact method completed in " << exact_timer.elapsed_seconds() << " seconds" << std::endl;
           
           // 2. 测试MCD/PCD方法
           std::cout << "Testing MCD/PCD method..." << std::endl;
           OptimizedMCDPCDCore mcd_pcd_core(graph_mcdpcd);
           mcd_pcd_core.reset(true);
           
           double mcd_pcd_remove_time = mcd_pcd_core.batch_remove(remove_edges);
           double mcd_pcd_add_time = mcd_pcd_core.batch_insert(add_edges);
           
           // 验证MCD/PCD方法的正确性
           bool mcdpcd_correct = CoreNumberValidator::validate_core_numbers(graph_exact, graph_mcdpcd);
           if (!mcdpcd_correct) {
               CoreNumberValidator::report_differences(graph_exact, graph_mcdpcd, "Exact", "MCD/PCD");
           }
           
           // 3. 测试UCR方法
           std::cout << "Testing UCR method..." << std::endl;
           OptimizedUCRCore ucr_core(graph_ucr);
           ucr_core.reset(true);
           ...
           // 验证UCR方法的正确性
           bool ucr_correct = CoreNumberValidator::validate_core_numbers(graph_exact, graph_ucr);
           if (!ucr_correct) {
               CoreNumberValidator::report_differences(graph_exact, graph_ucr, "Exact", "UCR");
           }
           
           // 4. 比较MCD/PCD和UCR结果的一致性
           if (mcdpcd_correct && ucr_correct) {
               std::cout << "Both methods produce correct core numbers!" << std::endl;
           } else if (mcdpcd_correct) {
               std::cout << "Only MCD/PCD method produces correct core numbers" << std::endl;
           } else if (ucr_correct) {
               std::cout << "Only UCR method produces correct core numbers" << std::endl;
           } else {
               // 比较两种方法之间的差异
               CoreNumberValidator::report_differences(graph_mcdpcd, graph_ucr, "MCD/PCD", "UCR");
           }
           ...
           std::cout << "MCD/PCD correctness: " << (mcdpcd_correct ? "PASS" : "FAIL") << std::endl;
           std::cout << "UCR correctness: " << (ucr_correct ? "PASS" : "FAIL") << std::endl;
       }
       ...
       // 正确性统计
       int mcdpcd_pass_count = 0;
       int ucr_pass_count = 0;
       
       for (const auto& result : results) {
           if (result.exact_matches_mcdpcd) mcdpcd_pass_count++;
           if (result.exact_matches_ucr) ucr_pass_count++;
       }
       
       std::cout << "MCD/PCD correctness: " << mcdpcd_pass_count << "/" << results.size() 
                << " (" << (100.0 * mcdpcd_pass_count / results.size()) << "%)" << std::endl;
       std::cout << "UCR correctness: " << ucr_pass_count << "/" << results.size() 
                << " (" << (100.0 * ucr_pass_count / results.size()) << "%)" << std::endl;
       ...
       // 将结果保存到CSV文件
       std::string csv_filename = "ucr_vs_mcdpcd_correctness_results.csv";
       ...
       if (csv_file.is_open()) {
           csv_file << "Step,RemoveBin,AddBin,NumRemoveEdges,NumAddEdges,"
                   ...
                   << "RemoveSpeedup,AddSpeedup,TotalSpeedup,"
                   << "MCD_PCD_Correct,UCR_Correct" << std::endl;
           ...
           for (const auto& result : results) {
               ...
               csv_file << result.step << "," 
                       ...
                       << add_speedup << "," 
                       << total_speedup << "," 
                       << (result.exact_matches_mcdpcd ? "1" : "0") << "," 
                       << (result.exact_matches_ucr ? "1" : "0") << std::endl;
           }
...
int main(int argc, char* argv[]) {
   ...
   std::cout << "=== UCR vs MCD/PCD Correctness Verification Experiment ===" << std::endl;
   ...
   // 创建实验运行器
   std::cout << "Starting correctness experiment..." << std::endl;
   Timer experiment_timer;
   CorrectnessExperimentRunner experiment(processor, window_size, starting_bin, num_steps, max_threads);
