...
public:
    ...
    // 批量处理边的插入
    double process_batch_insert(const vector<edge_t>& edges) {
        Timer timer;
    ...
    // 批量处理边的删除
    double process_batch_remove(const vector<edge_t>& edges) {
        Timer timer;
...
// 批量性能对比实验类
class BatchPerformanceExperiment {
private:
    string dataset_path;
    string dataset_name;
    
public:
    BatchPerformanceExperiment(const string& path, const string& name) 
    ...
    void run() {
        ...
        // 测试不同批量大小
        vector<int> batch_sizes = {1, 10, 100, 1000};
...
private:
    ...
    void run_batch_experiment(const Graph& base_graph, int batch_size) {
        ...
        Timer mcd_insert_timer;
        double mcd_insert_time = mcd_core.process_batch_insert(test_edges.first);
        
        Timer mcd_remove_timer;
        double mcd_remove_time = mcd_core.process_batch_remove(test_edges.second);
        
        LOG("  MCD/PCD Insert: " + to_string(mcd_insert_time) + " ms");
        LOG("  MCD/PCD Remove: " + to_string(mcd_remove_time) + " ms");
        
        // UCR测试
        Graph ucr_graph = base_graph.copy();
        UCRCore ucr_core(ucr_graph);
        
        Timer ucr_insert_timer;
        double ucr_insert_time = ucr_core.process_batch_insert(test_edges.first);
        
        Timer ucr_remove_timer;
        double ucr_remove_time = ucr_core.process_batch_remove(test_edges.second);
        
        LOG("  UCR Insert: " + to_string(ucr_insert_time) + " ms");
        LOG("  UCR Remove: " + to_string(ucr_remove_time) + " ms");
        
        // 计算加速比
        double insert_speedup = (mcd_insert_time > 0) ? mcd_insert_time / ucr_insert_time : 1.0;
        double remove_speedup = (mcd_remove_time > 0) ? mcd_remove_time / ucr_remove_time : 1.0;
        
        LOG("  Insert Speedup: " + to_string(insert_speedup) + "x");
        LOG("  Remove Speedup: " + to_string(remove_speedup) + "x");
    ...
    pair<vector<edge_t>, vector<edge_t>> generate_test_edges(const Graph& graph, int batch_size) {
        ...
        while (insert_edges.size() < batch_size) {
...
int main() {
    LOG("===========================================");
    LOG("Experiment 3: UCR vs MCD/PCD Batch Performance");
    LOG("===========================================");
    
    // 打开全局结果文件
    string output_file = "e3_results.txt";
    ...
    // 写入文件头
    global_results << "Dataset\tBatch_Size\tMCD_Insert(ms)\tUCR_Insert(ms)\tInsert_Speedup\t"
                   << "MCD_Remove(ms)\tUCR_Remove(ms)\tRemove_Speedup\n";
    global_results.flush();
    ...
    for (size_t i = 0; i < datasets.size(); ++i) {
        ...
        try {
            BatchPerformanceExperiment experiment(filepath, name);
            experiment.run();
