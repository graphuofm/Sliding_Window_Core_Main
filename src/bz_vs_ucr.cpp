...
private:
    ...
    void run_window_experiment(int window_size) {
        ...
        for (int slide = 0; slide < 10; ++slide) {
            ...
            LOG("    UCR remove time: " + std::to_string(remove_time) + " ms");
            LOG("    UCR insert time: " + std::to_string(insert_time) + " ms");
            ...
            // 保存结果
            results.push_back({dataset_name, window_size, slide + 1, bz_time, ucr_time});
...
int main() {
    LOG("===========================================");
    LOG("Sliding Window K-Core Experiment (BZ vs UCR)");
    LOG("===========================================");
    
    // 打开全局结果文件
    std::string output_file = "/home/jding/bz_vs_ucr_results.txt";
    ...
    // 写入文件头
    global_results << "Dataset\tWindow_Size\tSlide_Step\tBZ_Time(ms)\tUCR_Time(ms)\tSpeedup\n";
    global_results.flush();
