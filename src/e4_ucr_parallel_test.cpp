...
// 2. UCR-Parallel：并行处理连通分量
class UCRParallel : public UCRBase {
private:
    int num_threads;
    mutex graph_mutex;
    mutex values_mutex;
...
public:
    UCRParallel(Graph& g, int threads = 4) : UCRBase(g), num_threads(threads) {}
    ...
    double process_all(const vector<edge_t>& remove_edges, 
        ...
        // 处理删除
        process_removals(remove_edges);
        
        // 处理插入
        process_insertions(add_edges);
        
        return timer.elapsed_milliseconds();
    }
    
private:
    void process_removals(const vector<edge_t>& edges) {
        // Phase 1: 更新r值和删除边
        ...
        for (int k : k_values) {
            process_core_level_removal_parallel(k, k_groups[k]);
        }
    }
    
    void process_insertions(const vector<edge_t>& edges) {
        // Phase 1: 添加边
    ...
    void process_core_level_removal_parallel(int k, const unordered_set<vertex_t>& seeds) {
        // 找所有连通分量
        vector<vector<vertex_t>> components;
        unordered_set<vertex_t> visited;
    ...
    void process_component_removal(int k, const vector<vertex_t>& component) {
        ...
        while (!degrade_queue.empty()) {
            ...
            for (vertex_t w : G.adj[v]) {
                ...
                if (w < r_values.size() && G.core[w] > k - 1) {
                    lock_guard<mutex> lock(values_mutex);
                    r_values[w]--;
                }
            }
        }
    ...
    void process_component_upgrade_parallel(const vector<vertex_t>& component) {
...
private:
    ...
    static constexpr int NUM_BINS = 100;
    static constexpr int NUM_SLIDES = 3;
    
public:
    ParallelExperiment(const string& path, const string& name) 
    ...
    void run() {
        ...
        run_parallel_comparison(initial_graph, starting_bin);
    }
...
private:
    ...
    void run_parallel_comparison(Graph& initial_graph, int starting_bin) {
        LOG("\n--- Running parallel comparison ---");
        
        vector<double> basic_times, parallel2_times, parallel4_times, parallel8_times;
        ...
        for (int slide = 0; slide < NUM_SLIDES; ++slide) {
            ...
            // 2. UCR-Parallel (2 threads)
            {
                Graph ucr_graph = initial_graph.copy();
                UCRParallel ucr(ucr_graph, 2);
                double time = ucr.process_all(bins[remove_bin], bins[add_bin]);
                parallel2_times.push_back(time);
                LOG("    UCR-Parallel(2): " + to_string(time) + " ms");
            }
            
            // 3. UCR-Parallel (4 threads)
            {
                Graph ucr_graph = initial_graph.copy();
                UCRParallel ucr(ucr_graph, 4);
                double time = ucr.process_all(bins[remove_bin], bins[add_bin]);
                parallel4_times.push_back(time);
                LOG("    UCR-Parallel(4): " + to_string(time) + " ms");
            }
            
            // 4. UCR-Parallel (8 threads)
            {
                Graph ucr_graph = initial_graph.copy();
                UCRParallel ucr(ucr_graph, 8);
                double time = ucr.process_all(bins[remove_bin], bins[add_bin]);
                parallel8_times.push_back(time);
                LOG("    UCR-Parallel(8): " + to_string(time) + " ms");
            }
        ...
        if (!basic_times.empty()) {
            double avg_basic = 0, avg_p2 = 0, avg_p4 = 0, avg_p8 = 0;
            
            for (size_t i = 0; i < basic_times.size(); ++i) {
                avg_basic += basic_times[i];
                avg_p2 += parallel2_times[i];
                avg_p4 += parallel4_times[i];
                avg_p8 += parallel8_times[i];
            }
            
            avg_basic /= basic_times.size();
            avg_p2 /= basic_times.size();
            avg_p4 /= basic_times.size();
            avg_p8 /= basic_times.size();
            
            LOG("\nAverage times:");
            LOG("  UCR-Basic: " + to_string(avg_basic) + " ms");
            LOG("  UCR-Parallel(2): " + to_string(avg_p2) + " ms");
            LOG("  UCR-Parallel(4): " + to_string(avg_p4) + " ms");
            LOG("  UCR-Parallel(8): " + to_string(avg_p8) + " ms");
            
            LOG("\nSpeedups:");
            LOG("  2 threads: " + to_string(avg_basic / avg_p2) + "x");
            LOG("  4 threads: " + to_string(avg_basic / avg_p4) + "x");
            LOG("  8 threads: " + to_string(avg_basic / avg_p8) + "x");
...
vector<pair<string, string>> get_datasets(const string& dir) {
    ...
    if (dirp == nullptr) return datasets;
...
int main() {
    LOG("===========================================");
    LOG("Experiment 4: UCR Parallel Analysis");
    LOG("Testing: Basic vs Parallel (2,4,8 threads)");
    LOG("===========================================");
    
    string output_file = "/home/jding/e4_ucr_parallel_results.txt";
    ...
    global_results << "Dataset\tBasic(ms)\tParallel2(ms)\tParallel4(ms)\tParallel8(ms)\t"
    ...
    // 只测试前3个数据集
    int max_datasets = min(3, (int)datasets.size());
    ...
    for (int i = 0; i < max_datasets; ++i) {
        ...
        try {
            ParallelExperiment experiment(filepath, name);
            experiment.run();
