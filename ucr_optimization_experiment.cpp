...
// 批处理UCR - 基于原UCR稍作修改
class BatchUCRCore : public UCRCore {
public:
    BatchUCRCore(Graph& g) : UCRCore(g) {}
    
    double process_batch_removal(const std::vector<edge_t>& edges) {
        Timer timer;
        double total_time = 0;
        
        // 简单版本：逐个处理
        for (const auto& edge : edges) {
            total_time += process_edge_removal(edge.first, edge.second);
        }
        
        return timer.elapsed_milliseconds();
    }
    
    double process_batch_insertion(const std::vector<edge_t>& edges) {
        Timer timer;
        double total_time = 0;
        
        // 简单版本：逐个处理
        for (const auto& edge : edges) {
            total_time += process_edge_insertion(edge.first, edge.second);
        }
    ...
    void run_window_experiment(int window_size) {
        ...
        for (int slide = 0; slide < 10; ++slide) {
            ...
            for (const auto& edge : bins[remove_bin]) {
                remove_time += ucr_core.process_edge_removal(edge.first, edge.second);
            }
            
            double insert_time = 0;
            for (const auto& edge : bins[add_bin]) {
                insert_time += ucr_core.process_edge_insertion(edge.first, edge.second);
            }
            
            double total_time = ucr_timer.elapsed_milliseconds();
            
            LOG("  Base UCR - Remove: " + std::to_string(remove_time) + 
                " ms, Insert: " + std::to_string(insert_time) + 
                " ms, Total: " + std::to_string(total_time) + " ms");
            
            results.push_back({dataset_name, "Base UCR", window_size, slide + 1, 
                             remove_time, insert_time, total_time});
        ...
        for (int slide = 0; slide < 10; ++slide) {
            ...
            Timer ucr_timer;
            Graph ucr_graph = initial_graph.copy();
            BatchUCRCore ucr_core(ucr_graph);
            
            Timer remove_timer;
            double remove_time = ucr_core.process_batch_removal(bins[remove_bin]);
            
            Timer insert_timer;
            double insert_time = ucr_core.process_batch_insertion(bins[add_bin]);
            
            double total_time = ucr_timer.elapsed_milliseconds();
            
            LOG("  Batch UCR - Remove: " + std::to_string(remove_time) + 
                " ms, Insert: " + std::to_string(insert_time) + 
                " ms, Total: " + std::to_string(total_time) + " ms");
            
            results.push_back({dataset_name, "Batch UCR", window_size, slide + 1, 
                             remove_time, insert_time, total_time});
...
int main() {
    LOG("===========================================");
    LOG("UCR Optimization Experiment");
    LOG("Window size: 10 bins, 10 slides");
    LOG("===========================================");
    ...
    global_results << "Dataset\tAlgorithm\tWindow_Size\tSlide_Step\tRemove_Time(ms)\tInsert_Time(ms)\tTotal_Time(ms)\n";
    global_results.flush();
